...
Fixpoint count (v : nat) (s : bag) : nat :=
  ...
  | nil => 0
  | h :: t =>
      if v =? h then S (count v t) else count v t 
  end.
...
Fixpoint member (v : nat) (s : bag) : bool :=
  ...
  | nil => false
  | h :: t => 
      if h =? v then true else member v t 
  end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  ...
  | nil => nil 
  | h :: t => 
      if v =? h then t else h :: remove_one v t 
  end.
...
Theorem add_inc_count : forall (v : nat) (s : bag), S (count v s) = count v (add v s).
Proof.
...
Proof.
  ...
  - intros l2. simpl. rewrite <- app_assoc. rewrite <- IHl1. reflexivity.
Qed.
...
*)
Theorem bag_count_sum : forall v (s1 s2 : bag),
  count v (sum s1 s2) = count v s1 + count v s2.
Proof.
  intros v. induction s1.
  - intros s2. reflexivity.
  - intros s2. simpl. destruct (v =? n).
...
Proof.
  intros f Hinv. intros n1 n2 Ef.
  rewrite Hinv. rewrite <- Ef. rewrite <- Hinv. reflexivity.
Qed.
...
Theorem rev_injective : forall (l1 l2 : natlist),
  rev l1 = rev l2 -> l1 = l2.
Proof.
  intros l1 l2 Er. rewrite <- rev_involutive. rewrite <- Er.
...
Proof.
  intros [x]. simpl. rewrite eqb_refl. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros d x y o Hne. simpl. rewrite Hne. reflexivity.
Qed.
(** [] *)
End PartialMap.
