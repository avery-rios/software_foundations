...
Definition branch_case (P : booltree_property_type) : Prop :=
  forall b, forall t1, P t1 -> forall t2, P t2 -> P (bt_branch b t1 t2).
...
Check tree_ind : forall (X:Type) (P : tree X -> Prop),
  (forall x, P (leaf X x)) ->
  (forall t1, P t1 -> forall t2, P t2 -> P (node X t1 t2)) ->
  forall t, P t.
Check tree_ind.
(** [] *)
...
(** **** Exercise: 1 star, standard, optional (plus_explicit_prop)
    ...
    defined proposition.  *)
Definition P_add_assoc (n:nat) :=
  forall m p, n + (m + p) = n + m + p.

Check add_assoc' : forall n, P_add_assoc n.

Definition P_add_comm (n:nat) :=
  forall m, n + m = m + n.

Check add_comm' : forall n, P_add_comm n.
...
Definition better_t_tree_ind_type : Prop :=
  forall (X : Type) (P : t_tree X -> Prop),
    P t_leaf ->
    (forall l, P l -> forall x r, P r -> P (t_branch (l, x, r))) ->
    forall t, P t.
...
Definition better_t_tree_ind : better_t_tree_ind_type :=
  fun X P Pt Pb =>
    fix f (t : t_tree X) :=
      match t with
      | t_leaf => Pt
      | t_branch (l, x, r) => Pb l (f l) x r (f r)
      end.
...
Proof.
  ...
  - reflexivity.
  - intros l IHl x r IHr. simpl. rewrite IHl. rewrite IHr. reflexivity.
Qed.
